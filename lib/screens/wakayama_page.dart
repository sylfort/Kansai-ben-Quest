// lib/screens/wakayama_page.dart
...
class WakayamaPage extends StatefulWidget {
  const WakayamaPage({super.key});

  @override
  State<WakayamaPage> createState() => _WakayamaPageState();
}

class _WakayamaPageState extends State<WakayamaPage> {
  late Future<List<KansaiWord>> _wakayamaWordsFuture;

  @override
  void initState() {
    super.initState();
    _wakayamaWordsFuture = _loadWakayamaWords();
  }

  // Function to load, decode, and map Wakayama words JSON data
  Future<List<KansaiWord>> _loadWakayamaWords() async {
    try {
      // Load the specific JSON string for Wakayama from assets
      final String jsonString =
          await rootBundle.loadString('assets/data/wakayama_words.json');

      // Decode the JSON string into a List<dynamic>
      final List<dynamic> jsonList = json.decode(jsonString);

      // Map the dynamic list to a List<KansaiWord>
      final List<KansaiWord> wakayamaWords =
          jsonList.map((jsonItem) => KansaiWord.fromJson(jsonItem)).toList();

      return wakayamaWords;
    } catch (e) {
      // Handle errors (e.g., file not found, JSON parsing error)
      debugPrint('Error loading or parsing wakayama_words.json: $e');
      // Return an error to be caught by FutureBuilder
      return Future.error('Failed to load Wakayama words.');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          'Wakayama - 和歌山県', // Updated title for Wakayama
          style: TextStyle(
      ...
      body: FutureBuilder<List<KansaiWord>>(
        future: _wakayamaWordsFuture, // Use the future for Wakayama words
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError || !snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(
              child: Padding( // Add padding for error message
                ...
                child: Text(
                  snapshot.hasError
                      ? 'Error loading words.\nPlease check your connection or the data file (wakayama_words.json).' // Updated error message
                      : 'No words found for Wakayama in the dictionary.', // Updated message
          ...
          final wakayamaWords = snapshot.data!; // Assign data to specific variable
            ...
            child: SingleChildScrollView( // Horizontal scroll for the table content
              ...
              child: Padding(
                ...
                child: DataTable(
                  ...
                  rows: wakayamaWords.map((word) { // Map the list of words to DataRows
                    // Build each row
                    return DataRow(
