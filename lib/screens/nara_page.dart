...
class NaraPage extends StatefulWidget {
  const NaraPage({super.key});

  @override
  State<NaraPage> createState() => _NaraPageState();
}

class _NaraPageState extends State<NaraPage> {
  late Future<List<KansaiWord>> _naraWordsFuture;

  @override
  void initState() {
    super.initState();
    _naraWordsFuture = _loadNaraWords();
  }

  // Function to load, decode, and map Nara words JSON data
  Future<List<KansaiWord>> _loadNaraWords() async {
    try {
      // Load the specific JSON string for Nara from assets
      final String jsonString =
          await rootBundle.loadString('assets/data/nara_words.json');

      // Decode the JSON string into a List<dynamic>
      final List<dynamic> jsonList = json.decode(jsonString);

      // Map the dynamic list to a List<KansaiWord>
      final List<KansaiWord> naraWords =
          jsonList.map((jsonItem) => KansaiWord.fromJson(jsonItem)).toList();

      return naraWords;
    } catch (e) {
      // Handle errors (e.g., file not found, JSON parsing error)
      debugPrint('Error loading or parsing nara_words.json: $e');
      // Return an error to be caught by FutureBuilder
      return Future.error('Failed to load Nara words.');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          'Nara - 奈良県', // Updated title for Nara
          style: TextStyle(
      ...
      body: FutureBuilder<List<KansaiWord>>(
        future: _naraWordsFuture, // Use the future for Nara words
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError || !snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(
              child: Padding( // Add padding for error message
                ...
                child: Text(
                  snapshot.hasError
                      ? 'Error loading words.\nPlease check your connection or the data file (nara_words.json).' // Updated error message
                      : 'No words found for Nara in the dictionary.', // Updated message
          ...
          final naraWords = snapshot.data!; // Assign data to specific variable
            ...
            child: SingleChildScrollView( // Horizontal scroll for the table content
              ...
              child: Padding(
                ...
                child: DataTable(
                  ...
                  rows: naraWords.map((word) { // Map the list of words to DataRows
                    // Build each row
                    return DataRow(
