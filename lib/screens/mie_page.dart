...
class _MiePageState extends State<MiePage> {
  late Future<List<KansaiWord>> _mieWordsFuture;

  @override
  void initState() {
    super.initState();
    _mieWordsFuture = _loadMieWords();
  }

  // Function to load, decode, and map Mie words JSON data
  Future<List<KansaiWord>> _loadMieWords() async {
    try {
      // Load the specific JSON string for Mie from assets
      final String jsonString =
          await rootBundle.loadString('assets/data/mie_words.json');

      // Decode the JSON string into a List<dynamic>
      final List<dynamic> jsonList = json.decode(jsonString);

      // Map the dynamic list to a List<KansaiWord>
      final List<KansaiWord> mieWords =
          jsonList.map((jsonItem) => KansaiWord.fromJson(jsonItem)).toList();

      return mieWords;
    } catch (e) {
      // Handle errors (e.g., file not found, JSON parsing error)
      debugPrint('Error loading or parsing mie_words.json: $e');
      // Return an error to be caught by FutureBuilder
      return Future.error('Failed to load Mie words.');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<KansaiWord>>(
        future: _mieWordsFuture, // Use the future for Mie words
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError || !snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(
              child: Padding( // Add padding for error message
                ...
                child: Text(
                  snapshot.hasError
                      ? 'Error loading words.\nPlease check your connection or the data file (mie_words.json).' // Updated error message
                      : 'No words found for Mie in the dictionary.', // Updated message
          ...
          final mieWords = snapshot.data!; // Assign data to specific variable
            ...
            child: SingleChildScrollView( // Horizontal scroll for the table content
              ...
              child: Padding(
                ...
                child: DataTable(
                  ...
                  rows: mieWords.map((word) { // Map the list of words to DataRows
                    // Build each row
                    return DataRow(
